...
export class ApiHooks {
  ...
  public static async createSpaceByName(request: APIRequestContext, spaceName: string) {
    ...
    // Posting new space
    const response = await request.post(Endpoint.teamSpaces(), { headers: { Authorization: API_KEY }, data: newSpaceBody });
  }
  ...
  public static async createNewTask(request: APIRequestContext, taskName: string, taskListId?: string) {
    ...
    const response = await request.post(Endpoint.listTask(listId), { headers: { Authorization: API_KEY }, data: newTaskBody });
    return response;
  }
  ...
  public static async createNewTaskWithData(request: APIRequestContext, taskData: Object) {
    ...
    const response = await request.post(Endpoint.listTask(taskListId), { headers: { Authorization: API_KEY }, data: taskData });
  }
  ...
  public static async deleteTask(request: APIRequestContext, taskName: string) {
    ...
    await request.delete(Endpoint.task(taskId), { headers: { Authorization: API_KEY, "Content-Type": "application/json" } });
  }
  ...
  public static async deleteAllTasksFromList(request: APIRequestContext, listId: string) {
    const tasks = await request.get(Endpoint.listTask(listId), { headers: { Authorization: API_KEY } });
    const taskIds = (await tasks.json()).tasks.map((task) => task.id);

    for (const taskId of taskIds) {
      await request.delete(Endpoint.task(taskId), { headers: { Authorization: API_KEY } });
    }
  }
  ...
  public static async createNewDocInSpace(request: APIRequestContext, docName: string, spaceId: string) {
    ...
    await request.post(Endpoint.teamDocs(), { headers: { Authorization: API_KEY }, data: newDocBody });
  }
  ...
  public static async deleteFolderByName(request: APIRequestContext, spaceName: string, folderName: string) {
    ...
    await request.delete(Endpoint.folder(folderId), { headers: { Authorization: API_KEY } });
  }
  ...
  public static async createFolderByName(request: APIRequestContext, spaceName: string, folderName: string) {
    ...
    await request.post(Endpoint.spaceFolder(spaceId), { headers: { Authorization: API_KEY }, data: newFolderBody });
  }
}
