...
export function logClickingOnElement(elementName: string, specialKey?: string) {
  return function actualDecorator(originalMethod: any, context: ClassMethodDecoratorContext) {
    function replacementMethod(this: any, ...args: any[]) {
      console.log(`${getCurrentTimeAndDate()} - [${this.constructor.name}.${String(context.name)}] ${specialKey ?? ""} Clicking on ${elementName}`);
      const result = originalMethod.call(this, ...args);
      return result;
    }
...
export function logWaiting(message: string) {
  return function actualDecorator(originalMethod: any, context: ClassMethodDecoratorContext) {
    function replacementMethod(this: any, ...args: any[]) {
      console.log(`${getCurrentTimeAndDate()} - [${this.constructor.name}.${String(context.name)}] Waiting for: ${message}`);
      const result = originalMethod.call(this, ...args);
      return result;
    }
